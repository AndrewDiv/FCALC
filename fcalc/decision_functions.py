...
def alpha_weak_support(supp_cont, classes, class_lengths, alpha=0., randomize=False):
    ccl = class_lengths.sum() - class_lengths
    criter = np.zeros(shape=(len(class_lengths),supp_cont[0].shape[1]))
    preds = np.full(supp_cont[0].shape[1], -1.)
    for j in range(len(classes)):
...
def alpha_weak_density(supp_cont, classes, class_lengths, density, alpha=0., randomize=False):
    ccl = class_lengths.sum() - class_lengths
    criter = np.zeros(shape=(len(class_lengths),supp_cont[0].shape[1]))
    preds = np.full(supp_cont[0].shape[1], -1.)
    for j in range(len(classes)):
        criter[j] = ( density[j]*(supp_cont[j][1] <= ccl[j] * alpha) ).sum(axis=-1)
    if randomize:
        criter = criter.T / supp_cont[0].shape[-1]
    else:
        criter = criter.T / class_lengths
